...
@section('title', 'Order Detail — Farm Direct Wholesale')
...
<style>
    ...
    .progress-track {
        ...
        left: calc(10% + 30px);
        right: calc(10% + 30px);
        height: 2px; background: rgba(75,54,33,0.18); z-index: 0;
    }
    .progress-fill {
        position: absolute; top: 0; left: 0;
        height: 100%; background: var(--olive); transition: width 0.6s ease;
    }
    .progress-step {
    ...
    .step-dot--done    { background: var(--olive); box-shadow: 0 4px 14px rgba(128,128,0,0.3); }
    .step-dot--done i  { color: #fff; font-size: 28px; }

    .step-dot--current { background: var(--umber); box-shadow: 0 0 0 8px rgba(75,54,33,0.12), 0 4px 16px rgba(75,54,33,0.25); }
    .step-dot--current i { color: var(--champagne); font-size: 26px; }

    .step-dot--future  { background: var(--ivory); border: 2px solid rgba(75,54,33,0.2); }
    .step-dot--future i { color: rgba(75,54,33,0.3); font-size: 26px; }
    ...
    .step-label { font-size: 12px; letter-spacing: 0.04em; line-height: 1.4; font-weight: 400; }
    ...
    .items-table tfoot td:first-child { text-align: left; opacity: 0.5; font-weight: 400; font-size: 15px; }
...
<div class="detail-wrap">
    ...
    <a href="{{ route('wholesale.orders') }}" class="back-link">
    ...
    <div class="detail-header">
        <div>
            ...
            <p class="detail-placed">
                Placed {{ $order->created_at->diffForHumans() }} · {{ $order->created_at->format('d M Y, g:i A') }}
            </p>
        </div>
        @if(!$cancelled && $currentIndex !== false && $currentIndex < 3)
            <form action="{{ route('wholesale.orders.cancel', $order) }}" method="POST"
